...
实现PPT自动播放功能，只需要在每个页面的meta标签内设置好下一个页面的地址即可
...
每隔一分钟需要刷新页面的大屏幕监控，也可以使用meta，去掉后面的URL即可
...
+ 在即时通信场景比较麻烦
  ...
  一种低效解决方案：客户端通过轮询机制获取最新消息（HTML5下可使用WebSocket协议）

+ 消息提醒实现比较困难

  HTML5标准发布之前，浏览器没有开放图标闪烁，弹出系统消息之类的接口。

  只能通过借助一些Hack的手段，比如：修改title标签来达到类似的效果（HTML5下可使用Web Notifications API弹出系统消息）
  ...
  const inerval = setInterval(() => {
    ...
    const prefix = cnt % 2 ? `新消息(${msgNum})` : '';//1显示新消息 0不显示
    document.title = `${prefix}聊天页面`
  }, 1000)
  ```

实现效果：![](https://s0.lgstatic.com/i/image/M00/07/67/CgqCHl65LJGAR25PAAAXBLXRFXg133.gif)

动态修改 title 标签的应用场景:

+ 通过模拟消息闪烁，可以让用户在浏览其他页面的时候，及时得知服务端返回的消息。
+ 可以制作其他动画效果，比如文字滚动，但需要注意浏览器会对 title 标签文本进行去空格操作。
+ 将一些关键信息显示到标签上（比如下载时的进度、当前操作步骤），从而提升用户体验。
...
基于 meta 标签扩展属性值实现的第三方协议——OGP（Open Graph Protocal，开放图表协议 ）。
