// 1.创建AJAX实例对象（在IE5,IE6低版本浏览器中需要基于new ActiveXObject()处理）
let xhr = new XMLHttpRequest;
// 2.打开URL（发送请求前的一些处理）
xhr.open('get', './data.json', true); //请求的方法、请求的url地址、请求是否异步
// 3.监听AJAX状态信息
xhr.onreadystatechange = function() {
  // xhr.readyState  AJAX状态 0~4
  // xhr.status  xhr.statusText  服务器返回的网络状态码及描述  2/3/4/5
  /* if (xhr.readyState === 4 && xhr.status === 200) {
  	console.log(xhr.responseText);//response返回值的原始状态，不做任何处理；responseText把返回值以字符串形式返回
  } */
  if (xhr.status === 200) {
...
/* 
  xhr实例属性
  1、xhr.timeout=5000 设置超时时间为5000ms，超过设置的事件，请求自动中断，触发ontimeout事件，为0表示不设置
  2、xhr.upload 文件上传下载会用到
  3、xhr.withCredentials  CORS跨域资源请求中是否允许携带资源凭证
*/
/* 
  xhr原型上的方法
  1、xhr.open() 打开一个URL
  2、xhr.overrideMimeType() 覆盖mime类型
  3、xhr.setRequestHeader() 设置请求头
  4、xhr.getRequestHeader(key) 获取指定请求头
  5、xhr.getAllResponseHeaders() 获取所有请求头
  6、xhr.send()  发送请求
  6、xhr.abort()  强制中断请求 触发onabort事件
*/