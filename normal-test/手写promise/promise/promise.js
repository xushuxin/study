  ...
  if(typeof x==='object' && x!== null || typeof x === 'function') {
    ...
    try{//每次取then时都需要判断是否会报错（definedProperty定义的方法中可能会有逻辑导致不是每次取都取到）
      ...
      if(typeof then === 'function'){//判断then是不是一个函数
        //只能认准是x一个promise了
        then.call(x,y=>{//主动代用这个promise的then函数，获取这个promise的状态和结果
          if(called) return;
          called=true;
...
class Promise {
  constructor(executor) {
    ...
    try {//try+catch只能捕捉同步的错误
      //创建promise时立即执行
      executor(resolve, reject);//proimse中用户传入的回调函数，excecutor执行时 需要传入两个参数，给用户改变状态用
    } catch (err) {
  ...
  then(onFulFilled, onRejected) {
     ...
     * 总结：用户不传，我们帮他传；用户不捕捉错误，我们帮他捕捉错误抛给下一个promsie
     */
