...
7. #### loader使用
   ...
   + **url-loader \ file-loader：处理图片、音视频等资源**
...
10. #### webpack处理js
  ...
  需要借助Babel（把ES6+语法转为低级语法）[官网地址](https://www.babeljs.cn/)(插件下载)
...
注意：@babel/preset-env仅编译语句相关语法的插件，实例及实例上的新的语法不会处理

**某些语法(一般是处于提案阶段的语法)@babel/preset-env提供预设，则需要自己安装插件处理**
...
**2. class的装饰器语法需要安装Babel插件 @babel/plugin-proposal-decorators** 
...
**3. 处理async、await语法，Promise（全局变量）、Array.from（静态方法）、[].includes (实例方法)等语法**

corejs2仅支持转化全局变量及静态方法

corejs3支持实例方法

plugin-transform-runtime 配合runtime和runtime-corejs3可以转化绝大多数实例及实例方法，但是仍有部分语法不能转化
...
经测试，corejs3 目前不支持fetch的编译，或许可以尝试使用babel-polyfill
