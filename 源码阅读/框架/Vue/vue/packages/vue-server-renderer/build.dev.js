  ...
  for (i = 0, l = list.length; i < l; i++) {
    ...
    if (dirRE.test(name)) {
      ...
      if (bindRE.test(name)) { // v-bind
        ...
        name = name.replace(onRE, '');//获取v-on指令的名称
        isDynamic = dynamicArgRE.test(name);
        if (isDynamic) {
          name = name.slice(1, -1);
        }
        ...
        name = name.replace(dirRE, '');//获取v-on指令的名称
        // parse arg
...
function createCompileToFunctionFn (compile) {
  var cache = Object.create(null);//创建一个空对象，闭包保存
  ...
  return function compileToFunctions (
    ...
    /* istanbul ignore if */
    {
      ...
      try {
        //内容安全策略检查，不支持不安全的eval,提示用户放松策略或者预编译成render函数
    ...
    // compile
    var compiled = compile(template, options);//编译
