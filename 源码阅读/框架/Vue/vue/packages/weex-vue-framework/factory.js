...
function remove (arr, item) {
  if (arr.length) {//length存在且不为0
    var index = arr.indexOf(item);//查找索引
    if (index > -1) {
      return arr.splice(index, 1)//删除并返回该项
    }
  }
}
 ...
 */
//缓存fn执行的返回值，str每次传入相同时，返回值从缓存中获取
function cached (fn) {
 ...
 */
//只会匹配到-后面的第一个字符，如果str有多个-，会匹配每个-后面的第一个字符
var camelizeRE = /-(\w)/g;
var camelize = cached(function (str) {
  //把 a-bc-de 这种格式的字符串转为aBcDe这种小驼峰格式
 ...
 */
//把from的所有属性复制为to，返回to的引用(会覆盖同名属性)
...
var isServerRendering = function () {
  if (_isServer === undefined) {
    /* istanbul ignore if */
    //不在浏览器和微信，并且常量为server（entry-server-renderer.js中设置）,表示运行的编译指令是 npm run dev:ssr
...
Dep.target = null;
var targetStack = [];//用栈来维护Dep.target的指向，解决有嵌套computed的情况
...
var Observer = function Observer (value) {
  ...
  this.dep = new Dep();
  this.vmCount = 0;
  def(value, '__ob__', this);//定义__ob__属性保存观察者对象，后续可以根据该属性判断当前对象是否已被观察
...
function copyAugment (target, src, keys) {
  for (var i = 0, l = keys.length; i < l; i++) {
    var key = keys[i];
    //使用Object.defineProperty为target定义src中的指定的keys中指定的那些属性
...
function observe (value, asRootData) {
  ...
  //已被观察的对象、数组，不做操作，仅获取观察者对象返回（有__proto__属性，并且是Observer的实例）
  if (hasOwn(value, '__ob__') && value.__ob__ instanceof Observer) {
    ob = value.__ob__;
  } else if (//未被观察的对象、数组
    shouldObserve &&//没有禁用观察
    !isServerRendering() &&//不是服务器端渲染
    (Array.isArray(value) || isPlainObject(value)) &&//是数组或者普通对象
...
function defineReactive (
  ...
  var dep = new Dep();//创建dep对象，用于管理watcher
 ...
 * Object-based format.
 */
//把用户传入的props格式统一处理为对象返回
function normalizeProps (options, vm) {
  var props = options.props;
  if (!props) { return }
  var res = {};
  var i, val, name;
  //props支持两种格式：数组['prop1',...] 或者 对象格式{prop1:{...}}
...
function mergeOptions (
  ...
  if (typeof child === 'function') {//如果child是一个函数，获取到它的组件选项对象
    child = child.options;
  }

  //对用户传入的props、inject和directives进行格式处理，变成统一的格式
  normalizeProps(child, vm);//child.props最后都被处理为普通对象
  ...
  //child.options默认没有_base属性，只有Vue.options上或者合并过的options上有_base属性
  //意思就是：没有合并过才进行合并
  if (!child._base) {
    //如果child.options有extends配置项，需要把child.options.extends接收的组件选项对象合并到parent（新的对象）
    if (child.extends) {
      parent = mergeOptions(parent, child.extends, vm);
    }
     //如果child.options有mixins配置项，需要把child.options.mixins中所有的组件选项对象合并到parent（新的对象）
    if (child.mixins) {
      //从这里可以看出mixins必须是一个数组，并且每一项都是一个包含组件选项的对象
  ...
  for (key in parent) {
    //默认合并策略：
    //优先获取child.options中对应的属性和值，添加给最后返回的options对象
    //child.options中没有该属性(或者是undefined),则获取parent.options中的对应属性值，添加给返回的optios对象
    mergeField(key);
  }
  //再遍历child.options，把child.options中有，而parent.options中没有的属性值，添加给返回的options对象
  for (key in child) {
    //如果parent.options也有该属性，前面的遍历中就已经进行过mergeField了
    if (!hasOwn(parent, key)) {
      mergeField(key);
    }
  }
  function mergeField (key) {
    //strats默认是个空对象，所以使用的是默认的合并策略
...
function _traverse (val, seen) {
  var i, keys;
  var isA = Array.isArray(val);//是否为数组
...
//这个createElement会传递给render函数的一个参数，让用户使用
...
function renderMixin (Vue) {
  ...
  Vue.prototype._render = function () {
    ...
    try {
      ...
      //每个组件的render函数都会在这里调用，生成新的虚拟dom
      //vm._renderProxy在生产环境指向vm，也就是当前实例自身
...
function mountComponent (
  ...
  if (!vm.$options.render) {
    //如果没有传递render，创建一个空的VNode
    vm.$options.render = createEmptyVNode;
    if (process.env.NODE_ENV !== 'production') {
      /* istanbul ignore if */
      //默认没有模板编译功能时，如果传递了template，并且不是id选择器；或者组件配置对项中有el选项；或者直接传递了el
      //会提示使用预编译功能或者使用包含编译器的版本
  ...
  if (process.env.NODE_ENV !== 'production' && config.performance && mark) {
    ...
    //组件更新时触发的函数（默认会在dom更新之后执行）
    updateComponent = function () {
      //vm._render调用当前实例的render重新生成虚拟dom
      //vm._update会拿旧的虚拟dom和新的虚拟dom作对比，更新虚拟dom并触发页面渲染
  ...
  /**
   * vm当前组件实例，updateComponent更新组件时触发的函数，
   */
 ...
 */
//flushSchedulerQueue中主要是
//+ 调用一些钩子函数
//+ 按照添加顺序执行所有watcher的run方法
//  如果是renderWatcher,内部实际会调用updateComponent方法更新组件
//  如果是userWatcher,内部实际会调用用户传入的函数
function flushSchedulerQueue () {
  ...
  queue.sort(function (a, b) { return a.id - b.id; });//按照添加顺序排序
  ...
  for (index = 0; index < queue.length; index++) {
    watcher = queue[index];
    if (watcher.before) {
      watcher.before();//调用beforeUpdate钩子函数
    }
    id = watcher.id;
    has[id] = null;
    watcher.run();
  ...
  callActivatedHooks(activatedQueue);//activated钩子函数
  callUpdatedHooks(updatedQueue);//updated钩子函数
...
function queueWatcher (watcher) {
  var id = watcher.id;//获取watcher的id
  if (has[id] == null) {//如果不存在该watcher
    has[id] = true;//标记为已存在
    if (!flushing) {//如果不是正在刷新队列
      queue.push(watcher);//添加watcher到队列中
    } else {
      // if already flushing, splice the watcher based on its id
      // if already past its id, it will be run next immediately.
      var i = queue.length - 1;
      //index默认为0
      //找到队列中最开始的id大于当前watcher的id的那一项的索引
      while (i > index && queue[i].id > watcher.id) {
        i--;
      }
      //把当前watcher插入到那一项前面
      //i+1的原因是，找到了那一项后,i又减去了1，需要加回来
      queue.splice(i + 1, 0, watcher);
    }
    // queue the flush
    if (!waiting) {//waiting默认false
      waiting = true;
...
Watcher.prototype.cleanupDeps = function cleanupDeps () {
  ...
  var tmp = this.depIds;//缓存旧的存储依赖Id的Set
...
function initComputed (vm, computed) {
  ...
  var isSSR = isServerRendering();//是否为SSR，正常为false
  ...
  for (var key in computed) {
    var userDef = computed[key];//单个computed
    //获取到当前计算属性的getter
...
function initMixin (Vue) {
  ...
  Vue.prototype._init = function (options) {
    ...
    // a flag to avoid this being observed
    vm._isVue = true;//_isVue标记当前实例为Vue实例（避免被观测）
    // merge options
    if (options && options._isComponent) {
      // optimize internal component instantiation
      // since dynamic options merging is pretty slow, and none of the
      ...
      //合并内置options和用户传递的options
      vm.$options = mergeOptions(
        resolveConstructorOptions(vm.constructor), //vm.constructor=>Vue或者是其子类
        options || {},//用户传递的options对象
        vm
      );
    }
    /* istanbul ignore else */
...
//根实例初始化或者createComponent时都会调用这个方法，所以Ctor可能指向Vue或者其子类
function resolveConstructorOptions (Ctor) {
  //组件选项对象（如果是根组件，则为内置的options对象（注意不是用户传递的））
  //如果是子组件则是父子组件的options合并后的选项对象
  var options = Ctor.options;
  //如果是子组件的选项对象
  if (Ctor.super) {
    //获取其父类的options
    var superOptions = resolveConstructorOptions(Ctor.super);
    //extend时保存的父类的options
    var cachedSuperOptions = Ctor.superOptions;
    //判断父类的options是否发生改变
    if (superOptions !== cachedSuperOptions) {
      // super option changed,
      // need to resolve new options.
      Ctor.superOptions = superOptions;//设置为改变后的组件对象
...
function initUse (Vue) {
  ...
  Vue.use = function (plugin) {
    ...
    var args = toArray(arguments, 1);//获取插件对象之外的，传递的参数
...
function initExtend (Vue) {
  ...
  Vue.extend = function (extendOptions) {
    ...
    var Super = this;//Super => Vue或者其子类
    var SuperId = Super.cid;//获取父类的cid
    //首次会创建一个对象来缓存所有已创建的类
...
function createPatchFunction (backend) {
  ...
  var nodeOps = backend.nodeOps;//后端传递的数据
